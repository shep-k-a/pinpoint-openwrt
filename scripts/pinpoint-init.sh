...
# Detect if we're in Lite mode (no Python) or Full mode (with Python)
detect_mode() {
    ...
    stop)
        ...
        if nft list chain inet fw4 forward >/dev/null 2>&1; then
            ...
            if [ "$CURRENT_POLICY" = "drop" ]; then
                ...
                # Note: We can't change policy directly, but we can ensure it's ACCEPT via UCI
        ...
        log "Pinpoint stopped, settings restored"
        ;;
    restart)
        $0 stop
        sleep 1
